...
#[derive(Debug, Clone)]
pub enum GodotConditionCompilationError {
...
impl Into<venial::Error> for GodotConditionCompilationError {
    fn into(self) -> venial::Error {
        match self {
            Self::ListUnexpectedTokenInGroup(group, tt) => {
                let message = format!("Expected ident found in list '{}, {}'", group, tt);
                venial::Error::new_at_tokens(group, message)
            }
            Self::OptionUnsupported(ident) => {
                let message = format!("Conditional compilation option not supported '{}'", ident);
                venial::Error::new_at_tokens(ident, message)
            }
            Self::OptionInvalidToken(tt) => {
                let message = format!("Invalid token in conditional compilation option '{}'", tt);
                venial::Error::new_at_tokens(tt, message)
            }
            Self::PredicateExpectedIdentGotOther(tt) => {
                let message = format!("Unexpected token in conditional compilation predicate '{}'", tt);
                venial::Error::new_at_tokens(tt, message)
            }
            Self::PredicateInvalidGrouping(ident, group) => {
                let message = format!("Invalid grouping in conditional compilation predicate '{}{}'", ident, group);
                venial::Error::new_at_tokens(ident, message)
            }
            Self::PredicateInvalidAdjacentTokens(tt1, tt2) => {
                let message = format!("Invalid adjacent tokens in conditional compilation predicate '{}{}'", tt1, tt2);
                venial::Error::new_at_tokens(tt1, message)
            }
            Self::PredicateInvalidFormat(ts) => {
                let message = format!("Predicate format invalid '{}'", ts);
                venial::Error::new_at_tokens(ts, message)
            }
            Self::IncorrectNumberOfNotArguments(n, group) => {
                if n == 0 {
                    let message = format!("There must be one argument inside not()");
                    venial::Error::new_at_tokens(group, message)
                } else {
