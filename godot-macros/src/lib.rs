...
/// A property that has the same name as one of its base classes must declare this explicitly, with `override`:
///
/// ```no_run
/// # use godot::prelude::*;
/// #[derive(GodotClass)]
...
/// struct MyNode {
///     #[var(override)]
///     name: GString, // Shadows Node's `name` property.
/// }
/// ```
///
/// Under the `strict` [safeguard level](../index.html#safeguard-levels), the declaration is cross-checked against `ClassDB` at registration:
/// an error is reported if `override` is present without a matching base property, or absent while one exists.
///
