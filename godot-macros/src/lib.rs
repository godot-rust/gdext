...
///
/// ## Overriding base class properties -- `#[var(override)]`
///
/// When a property has the same name as a property in the base class, you must explicitly
/// mark it with `override` to indicate this is intentional:
///
/// ```
/// # use godot::prelude::*;
/// #[derive(GodotClass)]
...
/// struct MyNode {
///     #[var(override)]
///     name: GString,  // Overrides Node's built-in `name` property.
/// }
/// ```
///
/// This prevents accidental property shadowing and makes inheritance relationships clear. The attribute is validated at runtime during
/// class registration, and panics in the following cases:
/// - If `override` is present but the base class has no such property.
/// - If `override` is missing but the base class has the property.
///
