...
impl TestScriptLanguage {
    // In a real implementation the script would be created by the engine via ScriptLanguageExtension::create_script(), via a ResourceFormatLoader or by creating a new instance of TestScript. Most likely the script language would be registered as an engine singleton to make it globally available.
    #[func]
