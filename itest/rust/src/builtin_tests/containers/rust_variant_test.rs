...
fn rust_variant_godot_side_read_all_types() {
    ...
    // To make formatting also work in double builds (more digits), following vector has normal with <= 6 decimals and no whole numbers.
    verify_godot_side_read(
        Plane::new(Vector3::new(0.6, -0.64, 0.48), 2.5),
        "[N: (0.6, -0.64, 0.48), D: 2.5]",
    );
