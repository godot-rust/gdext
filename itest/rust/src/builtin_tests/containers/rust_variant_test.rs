...
use godot::builtin::{
    Array, Callable, Color, GodotStringExt, Plane, Quaternion, Rect2, Rect2i, Rid, Variant,
    VariantType, Vector2, Vector2i, Vector3, Vector3i, Vector4, Vector4i, varray,
};
...
fn rust_variant_setters_reject_complex_types() {
    ...
    let mut string_variant = Variant::from("hello".to_gstring());
...
fn rust_variant_readable_by_godot() {
    ...
    // Rust's own encode+decode round-trip could both use the same wrong data offset and still pass. Godot's
    // stringify() and Godot-evaluated `==` read the variant independently, so they catch a bad offset.
    assert_eq!(rust_variant.stringify().to_string(), "12345");
    assert_eq!(rust_variant, 12345_i64.to_variant());
}

// `Callable::invalid().call()` returns nil via the engine's own `callable_call` FFI function, independent of our
// zeroed-buffer `Variant::default()`/`Variant::nil()` path.
#[itest]
