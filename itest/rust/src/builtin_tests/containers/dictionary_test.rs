...
macro_rules! verify_key_value {
    (
        ...
        key: $key_pattern:pat,
        val: $value_pattern:pat,
    ) => {
        let __method = stringify!($method);
        let dict = $object.call(__method, &[]).to::<Dictionary>();

        let __key = dict.key_element_type();
        let $key_pattern = __key else {
            panic!(
                "{__method}():\n  expected key = {}\n  but was key =  {__key:?}",
                stringify!($key_pattern)
            );
        };

        let __val = dict.value_element_type();
        let $value_pattern = __val else {
            panic!(
                "{__method}():\n  expected value = {}\n  but was value =  {__val:?}",
                stringify!($value_pattern)
            );
        };
    };
}
...
"#,
    ...
    // 1) Dictionary.
    verify_key_value!(
        object.variant_variant_dict(),
        key: ElementType::Untyped,
        val: ElementType::Untyped,
    );

    // 2) Dictionary[String, Variant].
    verify_key_value!(
        object.builtin_variant_dict(),
        key: ElementType::Builtin(variant_type),
        val: ElementType::Untyped,
    );
    ...
    verify_key_value!(
        object.builtin_class_dict(),
        key: ElementType::Builtin(variant_type),
        val: ElementType::Class(class_name),
    );
    assert_eq!(variant_type, VariantType::COLOR);
    assert_eq!(class_name.to_string(), "RefCounted");

    // 4) Dictionary[Variant, CustomScriptForDictionaries].
    verify_key_value!(
        object.variant_script_dict(),
        key: ElementType::Untyped,
        val: ElementType::ScriptClass(script),
    );
