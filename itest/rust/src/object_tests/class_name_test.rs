...
fn class_id_new_dynamic() {
    let class_id = ClassId::new_dynamic("MyScriptClass".to_string());
    assert_eq!(class_id.to_string(), "MyScriptClass");
    assert_eq!(class_id, ClassId::new_dynamic("MyScriptClass".to_string()));
}
