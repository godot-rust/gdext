...
macro_rules! dyn_gd_obj {
    ($Trait:ident; $user_obj:expr) => {
        dyn_gd!($Trait; Gd::from_object($user_obj))
    }
}
...
macro_rules! dyn_gd {
    ($Trait:ident; $gd_instance:expr) => {{
        ...
        let downcast: Box<dyn Fn(&mut Gd<Object>) -> DynGdMut<_, dyn $Trait>> =
        ...
        DynGd::<_, dyn $Trait>::new(gd, downcast)
    }}
}
...
fn test() {
    ...
    // let mut dyn_gd = dyn_gd!(Health; user_obj);
