...
fn base_smuggling() {
    ...
    let extracted_base_obj = extracted_base.__fully_constructed_gd();
    ...
    expect_panic("accessing extracted base of dead object", || {
        extracted_base.__fully_constructed_gd().get_position();
    });
}
...
fn base_swapping() {
    ...
    assert_eq!(
        two.instance_id(),
        one_ext_base.__fully_constructed_gd().instance_id()
    );
