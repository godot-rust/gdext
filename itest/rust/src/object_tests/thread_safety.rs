...
fn sub_thread_surface_tool() {
    ...
    let result = std::thread::spawn(|| {
        ...
        let mesh = builder.commit_ex().existing(existing_mesh).done().unwrap();

        Unique::try_from_ref_counted(mesh)
            .expect("mesh must be unique when returned by SurfaceTool::commit")
    })
    .join()
