...
// `WeakRef::from_strong()` / `try_to_strong()` (a high-level wrapper API) never landed -- the merged PR #1560 shipped only
// documentation discouraging `WeakRef`, not the wrapper itself. So unlike the old `qol/weak-ref-tests` branch this predates,
// there is no `weakref_high_level_refcounted` test here; add one back if/when that API is implemented.
