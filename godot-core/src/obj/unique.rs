...
fn verify_unique_recursive(
    value: Gd<crate::classes::RefCounted>,
) -> Option<Gd<crate::classes::RefCounted>> {
    ...
    use crate::obj::{Gd, Singleton as _};

    match value.raw.ref_count() {
        0 => unreachable!(),
        1 => (),
        2.. => return None,
    }
    ...
    while let Some(next) = queue.pop_front() {
        ...
        // If the insert fails we have already checked the instance and ignore it.
        if !seen.insert(next.instance_id()) {
            continue;
        }

        // Sub resources should have a count of 2. One ref for the root-object and one for us.
        if next.raw.ref_count() > 2 {
            return None;
        }

        // Scripts could hold references to other objects.
        if next.get_script().is_some() {
            return None;
        }

        let api_type = class_db.class_get_api_type(&next.dynamic_class_string());
        ...
        for dict in next.__upcast_object().get_property_list().iter_shared() {
            for (_, property_value) in dict.iter_shared() {
                match property_value.get_type() {
                    ...
                    VariantType::OBJECT => {
                        let object = property_value.try_to::<Gd<RefCounted>>();
                    ...
                    VariantType::CALLABLE => {
                        let callable = property_value.try_to::<Callable>();
...
#[cfg(since_api = "4.4")]
impl<T: GodotClass + crate::obj::Bounds<Memory = bounds::MemRefCounted>> Unique<Gd<T>> {
    ...
    where
        T: GodotClass<Declarer = bounds::DeclEngine, Memory = bounds::MemRefCounted>
            + crate::obj::Inherits<crate::classes::RefCounted>,
    {
        verify_unique_recursive(value.upcast()).map(|verified| Self {
            inner: verified.cast(),
        })
    }
}
