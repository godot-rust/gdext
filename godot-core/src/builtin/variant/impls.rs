...
macro_rules! impl_ffi_variant {
    ...
    (ref $T:ty, $from_fn:ident, $to_fn:ident; $metadata:expr) => {
        impl_ffi_variant!(@ffi_only_impls by_ref, $metadata; $T, $from_fn, $to_fn);
    };
    ($T:ty, $from_fn:ident, $to_fn:ident; $metadata:expr) => {
        impl_ffi_variant!(@ffi_only_impls by_val, $metadata; $T, $from_fn, $to_fn);
    };
    (ref $T:ty, $from_fn:ident, $to_fn:ident) => {
        impl_ffi_variant!(@ffi_only_impls by_ref, ParamMetadata::NONE; $T, $from_fn, $to_fn);
    };
    ($T:ty, $from_fn:ident, $to_fn:ident) => {
        impl_ffi_variant!(@ffi_only_impls by_val, ParamMetadata::NONE; $T, $from_fn, $to_fn);
    };
    ...
    (@ffi_only_impls $by_ref_or_val:ident, $metadata:expr; $T:ty, $from_fn:ident, $to_fn:ident) => {
        impl GodotFfiVariant for $T {
            fn rust_to_variant(&self) -> Variant {
                unsafe {
                    Variant::new_with_var_uninit(|variant_ptr| {
