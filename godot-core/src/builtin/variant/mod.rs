...
use crate::builtin::{
    ExCall, GString, StringName, VarArray, VariantDispatch, VariantOperator, VariantType,
};
use crate::classes;
...
impl Variant {
    ...
    ///
    /// The bare [`call()`][Self::call] is shorthand for `call_ex(method).args(args).done()`. See [`ExCall`][crate::builtin::ExCall] for the
    /// available terminal operations.
    pub fn call_ex(&self, method: impl AsArg<StringName>) -> ExCall<'_> {
        crate::meta::arg_into_owned!(method);
        ExCall::on_variant(self, method)
    }
