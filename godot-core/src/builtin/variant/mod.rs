...
impl Variant {
    ...
    /// Enables conversions between related types that Godot considers compatible under its conversion rules, for example:
    /// - `i64` <-> `f64` <-> `bool`
    /// - `GString` <-> `StringName`
    /// - `GString` <-> `NodePath`
    /// - `Array` <-> `Packed*Array`
    ...
    ///
    /// Among Godot conversions, it uses the "strict" rules[^strict], meaning it does **not** convert between strings and `i64`, `f64`, `bool`.
    ///
    /// [^strict]: See [GDExtension interface](https://github.com/godotengine/godot/blob/4.4-stable/core/extension/gdextension_interface.h#L1353-L1364)
    ///   and [C++ implementation](https://github.com/godotengine/godot/blob/4.4-stable/core/variant/variant.cpp#L532) (Godot 4.4 at the time of writing).
    ///
