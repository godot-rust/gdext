...
/// through Godot's FFI layer. Internal type, not in the public API; reachable from test crates via the `__trace` feature.
///
/// # Memory management
...
impl RustVariant {
    ...
    /// overwriting such types without destruction. Gated on `trace` since mutable views (and `set_value`/`SetError`) are only used by `itest/`,
    ...
    /// Returns `true` if the variant currently holds "plain old data" (no destructor needed).
    ///
    /// Inverse of [`VariantType::needs_ffi_destruction()`]; used by `Variant`'s lifecycle fast paths (Clone/Drop) and `set_value`.
    #[inline]
    pub(crate) fn is_pod(&self) -> bool {
        !self.get_type_unchecked().needs_ffi_destruction()
    }
