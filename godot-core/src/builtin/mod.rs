...
//! correctness and performance. In particular, we leverage Rust's type system (such as `Option<T>` or `enum`) where this helps expressivity.
//!
...
//!
//! More `From` conversions have been requested a few times, often with vague reasons such as "would be useful" or "it's a common pattern in
//! Rust". People then often expect a detailed rationale why the trait is _not_ added, even though the burden of proof for every feature
//! generally lies with the person suggesting it, and `From` is no exception here.
//!
//! This section serves as a detailed explanation for the status quo, as to why `From` is relatively rare in godot-rust. This goes beyond the
//! builtin APIs, but can hopefully help you understand the underlying design decisions. This is not universal truth, other libraries are
//! well within their rights to choose differently here!
//!
//! 1. `From` next to named conversion methods/constructors adds another way to do things. While it's good to have some choice, multiple ways to
//!    achieve the same has downsides: Users wonder if there is a subtle difference between multiple options, or if they are in fact identical.
...
//!
//! One main reason to support `From` is to allow generic programming, in particular the `impl Into<T>` parameters. This is also the reason
