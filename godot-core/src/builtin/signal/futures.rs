...
/// [`Result::Err`]
pub struct SignalFuture<R: ParamTuple + Sync + Send>(TrySignalFuture<R>);
...
impl<R: ParamTuple + Sync + Send> RustCallable for SignalFutureResolver<R> {
    fn invoke(&mut self, args: &[&Variant]) -> Result<Variant, ()> {
        ...
        data.state = TrySignalFutureState::Ready(R::from_variant_array(args));
        drop(data);
...
pub struct TrySignalFuture<R: ParamTuple + Sync + Send> {
    data: Arc<Mutex<SignalFutureData<R>>>,
    callable: SignalFutureResolver<R>,
    signal: Signal,
}

impl<R: ParamTuple + Sync + Send> TrySignalFuture<R> {
    fn new(signal: Signal) -> Self {
        debug_assert!(
            !signal.is_null(),
...
impl<R: ParamTuple + Sync + Send> Future for TrySignalFuture<R> {
...
impl<R: ParamTuple + Sync + Send> Drop for TrySignalFuture<R> {
    fn drop(&mut self) {
...
impl Signal {
    ...
    pub fn to_try_future<R: ParamTuple + Sync + Send>(&self) -> TrySignalFuture<R> {
...
impl<C: WithBaseField, Ps: ParamTuple + Sync + Send> TypedSignal<'_, C, Ps> {
    /// Creates a fallible future for this signal.
    ///
    /// The future will resolve the next time the signal is emitted.
    /// See [`TrySignalFuture`] for details.
    pub fn to_try_future(&self) -> TrySignalFuture<Ps> {
        TrySignalFuture::new(self.to_untyped())
    }
    ...
    /// The future will resolve the next time the `Signal` is emitted, but might panic if the signal object is freed.
    /// See [`SignalFuture`] for details.
    pub fn to_future(&self) -> SignalFuture<Ps> {
        SignalFuture::new(self.to_untyped())
    }
}

impl<C: WithBaseField, Ps: ParamTuple + Sync + Send> IntoFuture for &TypedSignal<'_, C, Ps> {
    type Output = Ps;

    type IntoFuture = SignalFuture<Ps>;
