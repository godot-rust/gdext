...
use crate::builtin::{VarArray, Variant, VariantType, inner};
use crate::meta;
...
impl<K: ArrayElement, V: ArrayElement> Dictionary<K, V> {
    ...
    pub fn extend_dictionary(&mut self, other: &Self, overwrite: bool) {
        ...
        self.as_inner().merge(other.as_var_ref(), overwrite)
    }
    ...
    pub fn as_inner(&self) -> inner::InnerDictionary<'_> {
        inner::InnerDictionary::from_outer(self.as_var_ref())
    }
    ...
    fn as_var_ref(&self) -> &VarDictionary {
        // SAFETY: Dictionary<K, V> has the same memory layout as VarDictionary.
        // PhantomData and OnceCell are zero-sized or don't affect layout.
    ...
    fn init_inner_type(&mut self) {
        ...
        // Cache types on Rust side (for all versions).
...
impl<K: ArrayElement, V: ArrayElement> fmt::Debug for Dictionary<K, V> {
    fn fmt(&self, f: &mut fmt::Formatter<'_>) -> fmt::Result {
        write!(f, "{:?}", self.as_var_ref().to_variant().stringify())
    }
}
...
/// Helper: check if Dictionary<K, V> is fully untyped (both key and value are Variant).
#[inline]
