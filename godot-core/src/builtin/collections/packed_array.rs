...
use crate::meta::{AsArg, PackedArrayElement};
use crate::meta;
...
pub struct PackedArray<T: PackedArrayElement> {
    opaque: sys::types::OpaquePackedByteArray, // All packed arrays have same memory layout
...
impl<T: PackedArrayElement> PackedArray<T> {
    ...
    pub fn push(&mut self, value: impl AsArg<T>) {
        T::op_push_back(&mut self.as_inner(), value.into_arg());
    }

    // Potential for private API using value types (e.g. conversion/iterator collection functions).
    // fn push_owned(&mut self, value: T) {
    //     T::op_push_back(&mut self.as_inner(), CowArg::Owned(value));
    // }
    ...
    pub fn insert(&mut self, index: usize, value: impl AsArg<T>) {
        ...
        T::op_insert(&mut self.as_inner(), to_i64(index), value.into_arg());
    }
    ...
    pub fn fill(&mut self, value: impl AsArg<T>) {
        T::op_fill(&mut self.as_inner(), value.into_arg());
    }
    ...
    pub fn resize(&mut self, size: usize) {
        T::op_resize(&mut self.as_inner(), to_i64(size));
    }
    ...
    pub fn extend_array(&mut self, other: &PackedArray<T>) {
        ...
        T::op_append_array(&mut self.as_inner(), other);
    }
    ...
    pub fn subarray(&self, begin: usize, end: usize) -> Self {
        ...
        let opaque_slice: T::OpaqueType = T::op_slice(&self.as_inner(), to_i64(begin), to_i64(end));

        // SAFETY: All packed arrays have the same FFI memory layout, allowing safe reinterpretation between their opaque types.
        unsafe {
            let opaque_ptr = &opaque_slice as *const _ as *const sys::types::OpaquePackedByteArray;
            Self::from_opaque(*opaque_ptr)
        }
    }
    ...
    pub fn bsearch(&self, value: impl AsArg<T>) -> usize {
        ...
        to_usize(T::op_bsearch(&mut inner, value.into_arg(), true))
    }

    /// Reverses the order of the elements in the array.
    pub fn reverse(&mut self) {
        T::op_reverse(&mut self.as_inner());
    }
    ...
    fn ptr_or_none(&self, index: usize) -> Option<*const T> {
        ...
        let item_ptr: *const T::ReturnType = T::index_const(self.sys(), to_i64(index));
    ...
    fn ptr_mut(&mut self, index: usize) -> *mut T {
        // SAFETY: The packed array index operators return a null pointer on out-of-bounds.
        let item_ptr: *mut T::ReturnType = T::index_mut(self.sys_mut(), to_i64(index));
...
impl<T: PackedArrayElement + Clone> PackedArray<T> {
    ...
    pub fn remove(&mut self, index: usize) -> T {
        ...
        T::op_remove_at(&mut self.as_inner(), to_i64(index));
        element
    }
    ...
    fn default() -> Self {
        unsafe {
            Self::new_with_uninit(|self_ptr| {
                T::ffi_default(self_ptr as sys::GDExtensionTypePtr);
            })
        }
    }
}
...
impl<T: PackedArrayElement> Clone for PackedArray<T> {
    fn clone(&self) -> Self {
        unsafe {
            Self::new_with_uninit(|self_ptr| {
                T::ffi_copy(self_ptr as sys::GDExtensionTypePtr, self.sys());
            })
        }
    }
}
...
impl<T: PackedArrayElement> meta::GodotConvert for PackedArray<T> {
    type Via = Self;
}

impl<T: PackedArrayElement> meta::ToGodot for PackedArray<T> {
...
impl<T: PackedArrayElement> meta::FromGodot for PackedArray<T> {
...
impl<T: PackedArrayElement> meta::GodotType for PackedArray<T> {
    ...
    fn godot_type_name() -> String {
        // Use the element type to determine the array type name
        match T::VARIANT_TYPE {
            ...
            _ => panic!("Invalid variant type for PackedArray"),
        }
    }
}
    ...
    fn ffi_to_variant(&self) -> Variant {
        unsafe {
            Variant::new_with_var_uninit(|variant_ptr| {
                T::ffi_to_variant(self.sys(), variant_ptr as sys::GDExtensionVariantPtr);
            })
        }
    }
    ...
    fn ffi_from_variant(variant: &Variant) -> Result<Self, meta::error::ConvertError> {
        let array = unsafe {
            Self::new_with_uninit(|ptr| {
                T::ffi_from_variant(variant.var_sys(), ptr as sys::GDExtensionTypePtr);
            })
        };
        Ok(array)
    }
...
impl<T: PackedArrayElement + Clone> ops::Index<usize> for PackedArray<T> {
    type Output = T;
...
impl<T: PackedArrayElement + Clone> ops::IndexMut<usize> for PackedArray<T> {
...
impl<T: PackedArrayElement> Extend<T> for PackedArray<T> {
    fn extend<I: IntoIterator<Item = T>>(&mut self, iter: I) {
        ...
        const BUFFER_CAPACITY: usize = const_max(
            1,
            BUFFER_SIZE_BYTES / const { T::ELEMENT_SIZE },
        );
...
// Specific implementations for PackedByteArray conversion functions
