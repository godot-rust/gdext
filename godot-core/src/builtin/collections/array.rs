...
impl<T: ArrayElement> Array<T> {
    ...
    ///
    /// The comparator function should return an order code that indicates whether its argument is `Less`, `Equal` or `Greater` the desired
    /// target value. See also [`slice::binary_search_by()`].
    ///
        ...
        F: FnMut(&T) -> cmp::Ordering + 'static,
    {
        ...
        let rust_fn = |args| {
            ...
            let is_greater = matches!(func(&value), cmp::Ordering::Greater);

            Ok(is_greater.to_variant())
        };

        let debug_name = std::any::type_name::<F>();
        let index = Callable::with_scoped_fn(debug_name, rust_fn, |pred| {
    ...
    ///
    /// `pred` takes two elements `[a, b]` and should return if `a > b` (strictly greater).
