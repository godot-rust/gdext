...
impl OutArray {
    ...
    fn from_opaque(opaque: sys::types::OpaqueArray) -> Self {
        Self {
            array: VariantArray::from_opaque(opaque),
        }
    }
    ...
    /// Converts to `VariantArray` if the array is untyped.
    ///
    /// This is a shorthand for [`try_into_typed::<Variant>()`][Self::try_into_typed].
    pub fn try_into_untyped(self) -> Result<VariantArray, ConvertError> {
    ...
    pub(super) unsafe fn assume_type<T: ArrayElement>(self) -> Array<T> {
        debug_assert_eq!(self.array.element_type(), ElementType::of::<T>());
        self.array.assume_type()
    }
}
