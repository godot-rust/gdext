...
use crate::meta::{ArrayElement, ElementType, GodotConvert, GodotFfiVariant, GodotType, ToGodot};
...
///
/// `AnyDictionary` does not provide any operations where data flows _in_ to the dictionary, such as `set()` or `insert()`.
///
/// ## Conversions
...
#[derive(PartialEq)]
#[repr(transparent)] // Guarantees same layout as VarDictionary, enabling Deref from Dictionary<K, V>.
...
impl AnyDictionary {
    ...
    ///
    /// If you want to check for presence, use [`get()`][Self::get].
    ///
    /// # Panics
    ///
    ...
    pub fn contains_key<K: ToGodot>(&self, key: K) -> bool {
        self.dict.contains_key(&key.to_variant())
    }
    ...
    pub fn find_key_by_value<V: ToGodot>(&self, value: V) -> Option<Variant> {
        self.dict.find_key_by_value(&value.to_variant())
    }
    ...
    pub fn remove<K: ToGodot>(&mut self, key: K) -> Option<Variant> {
        self.dict.remove(&key.to_variant())
    }

    /// Alias for [`remove()`][Self::remove].
    ///
    /// _Godot equivalent: `erase`_
    pub fn erase<K: ToGodot>(&mut self, key: K) -> Option<Variant> {
        self.remove(key)
    }
    ...
    ///
    /// If `K=Variant` and `V=Variant`, this will attempt to "downcast" to an untyped dictionary,
    /// identical to [`try_cast_var_dictionary()`][Self::try_cast_var_dictionary].
    ///
    /// Returns `Err(self)` if the dictionary's dynamic types differ from `K` and `V`. Check
    /// [`key_element_type()`][Self::key_element_type] and [`value_element_type()`][Self::value_element_type]
    /// before calling to determine what types the dictionary actually holds.
    ///
...
impl Clone for AnyDictionary {
    fn clone(&self) -> Self {
        // We clone the inner VarDictionary.
        Self {
