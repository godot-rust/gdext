...
impl<P: FfiRawPointer> RawPtr<P> {
    ...
    ///
    /// # Safety
    /// You must guarantee that the pointer remains valid for the entire duration Godot may access the value you pass to it.
    /// Special care is necessary in `#[func]` and `I*` virtual function return types: if the pointer refers to a local variable,
    /// it will become immediately dangling, causing undefined behavior.
    #[inline]
