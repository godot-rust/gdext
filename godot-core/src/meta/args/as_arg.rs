...
macro_rules! impl_varg_variant {
    ...
    (forward [$($extra_gen:tt)*] $Pass:ty $(where $($extra_bound:tt)*)?) => {
        impl<$($extra_gen)* S, T> DisjointVArg<$Pass, T> for S
        where
            S: AsArg<T>,
            T: ToGodot<Pass = $Pass>,
            $($($extra_bound)*)?
    ...
    (ref_to_variant [$($GenericParams:tt)*] $T:ty) => {
        impl<$($GenericParams)*> DisjointVArg<ByVariant, Variant> for &$T {
            fn __disjoint_to_cow<'arg>(self) -> CowArg<'arg, Variant>
            where
                Self: 'arg,
            {
...
// Blanket forwards for pass strategies except ByVariant/ByRef, delegating to AsArg internally.
