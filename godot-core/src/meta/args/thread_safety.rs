...
impl<T: ToGodot> ThreadSafeArgContext for T
where
    <Self as ToGodot>::Threads: ThreadSafeArgAutoImpl<T>,
{
    fn guarantee_thread_safe(&self) {
        <Self as ToGodot>::Threads::validate_thread_safe();
    }
}

impl<T: ToGodot> ThreadSafeSealed for T where <Self as ToGodot>::Threads: ThreadSafeArgAutoImpl<T> {}
...
impl ThreadSafeArgContext for Variant {
    fn guarantee_thread_safe(&self) {
        if !self.get_type().is_thread_safe() && !crate::sys::is_main_thread() {
            ::std::panic!(
...
/// Argument is never thread-safe, the type will cause runtime panics when used outside the main thread.
///
/// See [`ToGodot::Threads`].
...
/// Argument is not always thread-safe, the type requires a custom implementation of [`ThreadSafeArgContext`] to perform runtime assertions.
///
/// See [`ToGodot::Threads`].
pub struct ManualThreadSafeArg;

impl ThreadSafety for ManualThreadSafeArg {}
impl Sealed for ManualThreadSafeArg {}

trait ThreadSafeArgAutoImpl<T> {
    fn validate_thread_safe();
}

impl<T: Send + GodotConvert> ThreadSafeArgAutoImpl<T> for ThreadSafeArg
where
    <T as GodotConvert>::Via: Send + Sync,
{
    fn validate_thread_safe() {}
}

impl<T> ThreadSafeArgAutoImpl<T> for NonThreadSafeArg {
    fn validate_thread_safe() {
        if !crate::sys::is_main_thread() {
            ::std::panic!(
                "Value of type {} is not known to implement the Send trait. You can use `#[godot(send)] for your `#[derive(GodotConvert)] if Send is implemented for your type. Or use `Threads = ThreadSafeArg` in your ToGodot impl.",
                std::any::type_name::<T>()
            );
        }
    }
