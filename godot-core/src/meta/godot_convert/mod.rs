...
/// `#[func]` methods (as they don't fit in GDScript/Variant). For internal use only; see `ToGodot` for user-facing conversions.
#[doc(hidden)]
...
///
/// This trait exists to support types like `u64` that work in engine contexts (backed by C++ `uint64_t`), but cannot be used in user-facing
/// `#[func]` methods (as they don't fit in GDScript/Variant). For internal use only; see `FromGodot` for user-facing conversions.
#[doc(hidden)]
