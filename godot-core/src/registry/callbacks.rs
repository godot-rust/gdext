...
///
/// `notify_postinitialize` is a plain `bool` rather than a callback: a closure parameter would add a generic axis, instantiating this function
/// twice per class (creation and hot reload).
    ...
    F: FnOnce(Base<T::Base>) -> T,
{
    ...
    // Before 4.4, Godot sends the notification itself when constructing the object.
    #[cfg(before_api = "4.4")]
