...
fn make_enum_values_and_constants_functions(enum_: &Enum) -> TokenStream {
    ...
    for (index, enumerator) in enum_.enumerators.iter().enumerate() {
        let ordinal = match &enumerator.value {
            EnumeratorValue::Enum(ord) => *ord,
            EnumeratorValue::Bitfield(ord) => *ord as i32,
        };
        ...
        if enum_.max_index != Some(index) && seen_ordinals.insert(ordinal) {
