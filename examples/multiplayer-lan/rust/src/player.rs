...
impl Player {
    ...
    fn fire(&self) {
        ...
        bullet.set_global_position(
            self.base()
                .get_node_as::<Node2D>("GunRotation/BulletSpawn")
                .get_global_position(),
        );
        bullet.set_rotation_degrees(
            self.base()
                .get_node_as::<Node2D>("GunRotation")
    ...
    */
    ...
    fn on_player_body_entered(&mut self, body: Gd<PhysicsBody2D>) {
        if let Ok(mut bullet) = body.try_cast::<Bullet>() {
            ...
            if bullet_id != self.get_network_id() {
                self.base_mut().rpc(
                    "take_damage".into(),
                    &[Variant::from(bullet_id), Variant::from(1)],
                );
            }
            bullet.queue_free();
        }
    ...
    fn take_damage(&mut self, attacker_id: NetworkId, damage: i32) {
        ...
        if self.health <= 0 {
            ...
            self.base_mut()
                .emit_signal("death".into(), &[Variant::from(network_id)]);
        }
    }
}
...
impl ICharacterBody2D for Player {
    ...
    fn ready(&mut self) {
        ...
        hurt_box.connect(
            "body_entered".into(),
            self.base().callable("on_player_body_entered"),
        );
        /*
    ...
    fn physics_process(&mut self, delta: f64) {
        ...
        if self.multiplayer_synchronizer.get_multiplayer_authority()
